...
Public Class HistorialMovimientosExtras
    ...
    Private Sub actualizarMonto()
        ...
        da.Fill(ds, "caja")
        txtMontoActual.Text = "Saldo Caja: $" & ds.Tables("caja").Rows(0)("Monto")
    ...
    Private Sub HistorialMovimientosExtras_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        cargaventas("SELECT TOP (1000) [ID_ME] as ID, [FECHA], [CONCEPTO], CASE [ING] WHEN 'SI' then [Monto] ELSE ([MONTO]*(-1)) END AS Importe FROM [TPFinal].[dbo].[MovExtras]")
    ...
    Private Sub btnBuscar_Click(sender As Object, e As EventArgs) Handles btnBuscar.Click
        Dim query, fecha, fecha2 As String
        'Se declara la consulta dejando la condicion
        query = "SELECT TOP (1000) [ID_ME] as ID, [FECHA], [HORA], [CONCEPTO], CASE [ING] WHEN 'SI' then [Monto] ELSE ([MONTO]*(-1)) END AS Importe FROM [TPFinal].[dbo].[MovExtras] WHERE "
        ...
        If rbFecha.Checked Then
            ...
            fecha2 = Format(Calendario2.Value, "yyyy/MM/dd").ToString 'Se obtiene la fecha seleccionada en el panel de fecha
            cargaventas(query & "[FECHA] BETWEEN '" & fecha & "' AND '" & fecha2 & "'")
